...
export default function WishlistPage() {
  ...
  const toggleWishlistMutation = useMutation({
    mutationFn: async (bookId: string) => {
      const res = await api.post("/users/me/wishlist", { bookId });
      return res.data;
    },
    onSuccess: () => {
  ...
  if (isLoading) {
    return <p className="animate-pulse">Loading wishlist...</p>;
  }
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-gray-500">Books you plan to buy or read in the future.</p>
      </div>

      {wishlist.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-20 border rounded-2xl bg-white dark:bg-gray-950/20">
          <Bookmark className="h-12 w-12 text-gray-300 dark:text-gray-700 mb-3" />
          <p className="font-bold text-gray-600 dark:text-gray-400">Wishlist is empty</p>
          <p className="text-xs text-gray-400 mt-1">Bookmark books to remember to explore them later.</p>
          ...
          {wishlist.map((item: any) => {
            ...
            return (
              ...
              >
                ...
                >
                  <Trash2 className="h-3.5 w-3.5" /> Remove Wishlist
                </button>
              </motion.div>
            );
          })}
